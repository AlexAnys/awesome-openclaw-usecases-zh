...
跨多个 Microsoft 365 租户的智能邮件监控，通过成本优化的预取（prefetch）架构将 token（词元）消耗降低 99%，再由智能分拣决定哪些邮件值得提醒你——哪些可以等一等。

## 痛点

如果你用 OpenClaw 监控邮件，大概率踩过这些坑：

- **成本爆炸**：每次 cron job（定时任务）触发都会写入一次完整的系统提示词（prompt）缓存。如果每 15 分钟检查一次邮件，一天就是 96 次冷启动——智能体还没读邮件就已经开始烧钱了。用 Claude Sonnet 每次冷启动约 $0.20，光邮件检查每天就要 $20。
...
- [上游用例 PR](https://github.com/hesamsheikh/awesome-openclaw-usecases/pull/46) — 原始英文版本
- [mcporter 技能](https://playbooks.com/skills/openclaw/skills/mcporter) — Microsoft 365 MCP 集成
- [邮件收件箱清理](inbox-declutter.md) — 事后清理型邮件管理用例（互补方案）
...
| **腾讯企业邮箱** | IMAP（邮件访问协议）/SMTP + 开放 API | 低 | 企业微信生态用户 | 推荐 |
...
| 原版方案 | 国内替代 | 接入方式 | 推荐场景 |
|---------|---------|---------|---------|
| Telegram | **企业微信群机器人** | Webhook（网络钩子），最简单 | 企业微信用户 |
| Telegram | **飞书群机器人** | Webhook，支持富文本卡片 | 飞书用户 |
| Telegram | **钉钉群机器人** | Webhook，支持 Markdown（标记语言） | 钉钉用户 |
