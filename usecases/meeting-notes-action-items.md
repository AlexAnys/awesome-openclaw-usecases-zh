...
> **👤 人机分工（最小必要人工 · 时点 · 凭证）**
> - **一次性（开始前）**：选定会议 / 任务 / 沟通平台组合（业务决策）；完成各平台**账号授权**并提供转录与 API 凭证。
> - **付费 / 门槛**：腾讯会议转写 API 需商业版 / 企业版；Jira / Linear 等视团队订阅。
> - **周期性 / 自动**：进阶管道每 30 分钟自动扫描转录文件夹并处理；人工仅在 token/密钥过期续期时介入。
> - **事件触发**：每次开会后从飞书妙记/钉钉导出转录文本（或直接粘贴）即触发处理；腾讯会议可由 Agent 通过开放平台 API 自动拉取转写与纪要，无需人工导出。
> - **外发前确认**：创建任务、向 Slack/飞书群聊发布纪要摘要、定时催办负责人等对外动作需用户确认。
> - **凭证**：腾讯会议 API 凭证、飞书应用密钥（通过环境变量或 `.env` 配置）；视所选平台另需 Jira/Linear/Slack 等对应凭证。
>
> 装技能（`npx` 一行命令）/ 配置管道等能力项不列在此——由你的 agent 现场探测并代办（协议见 [AGENTS.md](../AGENTS.md)）。
