...
飞书官方于 2026-03-28 开源了 [Lark CLI](https://github.com/larksuite/cli)（MIT），这是一个命令行工具，覆盖飞书 2500+ API、11 个业务领域，内置 19 个 AI Agent Skills。装好之后，你的 AI Agent 就能以你的身份操作飞书。
...
不需要额外的 OpenClaw 技能。Lark CLI 自带 19 个 AI Agent Skills，安装时会自动 symlink 到 OpenClaw / Claude Code / Cursor 等 Agent 框架。
...
- **Token 有效期**：Access Token 2 小时（自动续期），Refresh Token 7 天。超过 7 天没用需要重新授权一次——跟 Agent 说"重新授权飞书"就行
- **输出格式**：对 Agent 来说 `--format json`（默认）最好解析；对人来说 `--format pretty` 或 `--format table` 最直观
- **`--dry-run`**：在执行可能产生副作用的操作前（如发消息、改记录），可以用 `--dry-run` 先预览请求
