...
> **👤 人机分工（最小必要人工 · 时点 · 凭证）**
> - **一次性（开始前）**：在浏览器打开授权链接、用飞书账号确认 OAuth 授权。
> - **付费 / 门槛**：无（Lark CLI 为飞书官方开源，MIT 协议）。
> - **周期性 / 自动**：不适用；Access Token 2 小时自动续期，仅当超过 7 天未用、Refresh Token 失效时，跟 Agent 说"重新授权飞书"手动重授一次。
> - **外发前确认**：发消息、改多维表格记录等有副作用的操作建议先用 `--dry-run` 预览再执行。
> - **凭证**：`Access Token`、`Refresh Token`（OAuth 用户身份令牌，无需手填 API Key）。
>
> 装 CLI / 装技能 / 写配置等能力项不列在此——由你的 agent 现场探测并代办（协议见 [AGENTS.md](../AGENTS.md)）。
