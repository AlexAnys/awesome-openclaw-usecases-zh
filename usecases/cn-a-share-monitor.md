...
> **👤 人机分工（最小必要人工 · 时点 · 凭证）**
> - **一次性（开始前）**：选数据源（业务决策：AKShare / cn-a-stock MCP / 可选 Tushare Pro）；完成飞书/钉钉/企业微信推送的**账号授权**（参考对应 IM 集成用例）；需分钟级数据时注册 Tushare Pro 并提供 `TUSHARE_TOKEN`。
> - **付费 / 门槛**：AKShare 免费、无需 Key（部分接口对海外 IP 有限制）；分钟级数据需 Tushare Pro。
...
>
> `pip install akshare` / 添加 MCP 配置 / 建 cron 等能力项不列在此——由你的 agent 现场探测并代办（协议见 [AGENTS.md](../AGENTS.md)）。
