...
- **注意数据流向**：不同方案的隐私级别不同——
  - **纯本地方案**（方案一聊天记账、方案三 CSV 导入、方案四 ezBookkeeping 自托管）：数据存储在本地 SQLite 或自托管服务器，不经过第三方云端
  - **云通道方案**（方案二收据 OCR、飞书/Telegram/钉钉推送）：记账消息和收据图片会经过 Telegram/飞书等第三方服务器，个人财务数据对这些平台可见
  - 对隐私要求较高的场景，建议优先使用方案一或方案四，避免通过即时通讯平台传输敏感财务信息
...
> 💡 如果你只需要移动端记账，咔皮记账等原生 App 可能更方便。OpenClaw 方案的优势在于**高度定制化**（自定义分类规则、跨平台数据整合、自动化报表）和**灵活的部署选择**（纯本地方案可实现数据完全自主可控，云通道方案则需注意数据会经过第三方服务器）。
