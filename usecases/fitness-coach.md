...
将以下 MCP 配置添加到你的客户端配置文件中：

- **OpenClaw**：`~/.openclaw/openclaw.json` 的 `mcpServers` 字段
...
> 路径取决于你使用的客户端。OpenClaw 使用 `~/.openclaw/commands/`，Claude Code / Claude Desktop 使用 `~/.claude/commands/`。
...
在技能文件（`fitness-coach.md`）中更新 Obsidian 库路径，指向你的实际目录。
