...
> **👤 人机分工（最小必要人工 · 时点 · 凭证）**
> - **一次性（开始前）**：完成 IM 通道（飞书 / Telegram / Discord / iMessage）与任务管理器（Todoist / Apple 提醒事项 / Asana）的**账号授权**，并告诉 agent 你的新闻源偏好。
> - **付费 / 门槛**：无（常用 IM 与任务管理器免费档即可）。
> - **周期性 / 自动**：cron 每工作日 7:30（`30 7 * * 1-5`，Asia/Shanghai；原版每日 8:00）自动生成并推送简报；人工仅在 IM / 任务管理器授权过期需重新登录时介入。
> - **事件触发**：不适用。
> - **外发前确认**：简报自动发送至 IM 属预期推送、无需逐次确认；若采纳"AI 建议帮你自动完成的任务"，相关对外动作（发送 / 发布 / 下单等）应在执行前确认。
> - **凭证**：无（IM 与任务管理器均通过集成 / OAuth 授权，正文未指定具体 Key 名）
>
> 安装集成 / 浏览器与搜索技能 / 建 cron 等能力项不列在此——由你的 agent 现场探测并代办（协议见 [AGENTS.md](../AGENTS.md)）。
