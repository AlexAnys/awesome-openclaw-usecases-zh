...
> **👤 人机分工（最小必要人工 · 时点 · 凭证）**
> - **一次性（开始前）**：把 OpenClaw 连接到你偏好的消息平台（Telegram / iMessage / Discord），完成集成授权。
> - **付费 / 门槛**：无。
> - **周期性 / 自动**：不适用（无 cron 定时任务）。
> - **事件触发**：每当你想到内容（路上 / 会议中 / 睡前）给机器人发消息时即时捕获。
> - **外发前确认**：无对外动作（记忆写入个人记忆系统、仪表盘仅供本人浏览）。
> - **凭证**：消息平台集成授权（Telegram / iMessage / Discord 的集成令牌 / OAuth）。
>
> 构建部署仪表盘等能力项不列在此——由你的 agent 现场探测并代办（协议见 [AGENTS.md](../AGENTS.md)）。
