...
> **👤 人机分工（最小必要人工 · 时点 · 凭证）**
> - **一次性（开始前）**：注册 Perplexity 与 Firecrawl 账号并获取两个 API Key；提供要监控的竞品清单（业务判断，由人决定监控谁）；如需推送，在飞书后台创建机器人并取得 Webhook。
> - **付费 / 门槛**：免费层即可起步（Perplexity 每天 5 次搜索、Firecrawl 每月 500 次抓取），超量才需付费。
...
>
> 配置 MCP / 写监控列表 / 建命令与 cron 等能力项不列在此——由你的 agent 现场探测并代办（协议见 [AGENTS.md](../AGENTS.md)）。
