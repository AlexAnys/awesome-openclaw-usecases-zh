...
> **👤 人机分工（最小必要人工 · 时点 · 凭证）**  ← 推荐，新用例尽量内置
> - **一次性（开始前）**：仅列四类不变量——注册 / OAuth 授权 / 提供 Key / 业务选择
> - **付费 / 门槛**：付费档位、企业认证、平台准入限制等（必填，无则写"无"）
> - **周期性 / 自动**：cron 自动跑的频率；人工仅在 token 续期等（一次性任务写"不适用"）
> - **事件触发**：由什么事件触发（无则省略此行）
> - **外发前确认**：哪些对外动作需用户确认（纯本地 / 只读写"无对外动作"）
> - **凭证**：`KEY_A`、`KEY_B`（无则写"无"）
>
> XX / YY 等能力项不列在此——由执行 agent 现场探测并代办（见 AGENTS.md）
>   ↑ 结尾固定加这行；装 skill / 写配置 / 建 cron 等能力项**不写进块**——它们是运行时事实，静态文档无法预判执行 agent 的环境
...
- 「👤 人机分工」块让读者和 AI agent 一眼看清"自己最少要做什么、在什么时点做"。高价值用例建议内置；其余用例可省略，agent 会按 [AGENTS.md](AGENTS.md) 的人机分工协议现场推导。标签词请沿用固定写法（一次性（开始前）/ 周期性 / 自动 / 事件触发 / 外发前确认 / 凭证）以便机器解析。
- "必须人工"按 AGENTS.md 的四类判定：身份/授权、凭证、决策、物理动作；shell 安装、写文件、改配置、建 cron 等能力项**一律不写进块**，由用户 agent 运行时探测代办（沙箱受限时由其现场回落）。「付费 / 门槛」为必填槽位——这是读者决定"要不要开始"的最贵信息，必须前置可见。
