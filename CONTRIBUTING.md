...
> - **一次性（开始前）**：仅列用户必须亲自做的——注册 / OAuth 授权 / 提供 Key / 做选择；装 skill、改配置等 agent 可代办的注明"可交给 agent"
...
- "必须人工"按 AGENTS.md 的四类判定：身份/授权、凭证、决策、物理动作；shell 安装、写文件、改配置、建 cron 默认 agent 代办（用户 agent 沙箱受限时由其现场回落）。
