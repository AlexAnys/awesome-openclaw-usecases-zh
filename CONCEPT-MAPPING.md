...
把 OpenClaw 的核心概念**近似映射**到其他主流个人 AI 智能体生态。本仓库 50 个用例都用 OpenClaw 术语写就，遇到不熟悉的术语回到这张表查找等价物即可，**无需改写用例本体**。
...
|---|---|---|---|---|
| **Skill**（教智能体做某事的知识包） | `~/.hermes/skills/<cat>/<skill>/SKILL.md` | `.claude/commands/*.md`（首选;项目级 slash 命令,启动时自动加载） | `~/.agents/skills/`（个人）/ `<repo>/.agents/skills/`（团队） | Claude Code 的 `.claude/skills/` 需要插件注册,**`.claude/commands/` 可直接 drop-in 生效**(经实测)。 |
| **Cron Job / Heartbeat**（按时间表执行 / 定期巡检） | Hermes cron + closed learning loop | `/schedule` 创建持久化远程 routine（跨会话存活）;也可用系统 cron 触发 `claude -p` | Codex automations（Web / Cloud 计划任务） | Claude Code `/schedule` 是原生持久 cron,无需外部调度器。 |
...
| **AGENTS.md**（智能体操作手册） | 会读取启动目录下的 `AGENTS.md`；建议从仓库根启动以保证根上下文加载 | 不按文件名自动扫描,但 `CLAUDE.md` 的 `@AGENTS.md` 显式 import 原生支持(本仓库已采用) | 自动读取（CLI / Web / Cloud） | 跨工具事实标准之一；Hermes 不做 Codex 那种目录树多层合并。 |
...
- 「设置 Cron / Heartbeat」→ Claude Code 用 `/schedule` 创建持久 routine;Hermes 用 `cronjob`;也可退化到系统 cron / launchd。
