...
**第一步：推导人机分工。** 用例正文是唯一事实来源——你据它推导出**不变量**（授权 / 凭证 / 决策 / 付费门槛 / 外发确认，以及用户侧时点），这是必须交给用户判断的部分；而**能力项（装 skill / 写配置 / 建 cron 等）永远不推给用户**，无论用例怎么写，都由你现场探测（有无 shell / cron / MCP、`which npx` 等）后代办或回落。按下表从正文推导不变量：
...
其余——shell 安装（`npx` / `pip` / `brew`）、写文件、改配置、建 cron——**默认由你代办**。**回落前置门槛**：只有当你**实际跑过**探测/安装命令（如 `which npx`、`npm --version`、试跑安装）且它**真的报错**时，才可把该动作移回"需用户协助"，并在二次指导中**贴出失败命令与报错原文**作为证据；**严禁仅凭"本会话没有 shell / 终端"这类自述就回落**——先试，失败有据才退。回退时按三段说清：『此项本应我代办 → 因〈贴上失败命令的报错〉 → 故仅本轮请你在可写环境跑：…』，不得只甩裸命令清单（否则用户无法区分"环境硬限"与"你偷懒"）。
**自检（双向）**：① 把任何一项标为"需用户"之前，先答"它属于四类之一吗？"——不属于就探测后代办，不得以"界面要点击 / 看起来麻烦"为由推给用户；② 把任何③类业务决策自设默认值往下做之前，先答"用户给过真实值吗？"——没有就先问，不得用占位符代填后径直推进。

**第二步（动手前的硬门）：先把下面这份清单发给用户，再开始第三步里任何写文件 / 改配置 / dry-run。** 它是事前契约、不是事后总结，也是你这一轮唯一的提问批次（之后只在"真卡住"时追加一次）：
...
**第三步：按"先探测、一次问齐、卡住才追加"的时序执行。**
① **只读探测**（仅探测、不写文件 / 不 dry-run）：实测自身能力——试写一个 /tmp 临时文件、`crontab -l`、列 MCP 工具、`which npx / node`——探到能定位卡点即可，不为"摸全环境"无界消耗；
...
④ **真卡住才追加一次**；外发动作始终停在确认点。
遇到你这个 agent 没有原生等价能力的步骤（如 OpenClaw 的 Channel / Cron），按 [CONCEPT-MAPPING.md](CONCEPT-MAPPING.md) 找替代落地方式后再继续。

> 这套推导规则对**任何模型 / 任何 agent**通用：用例正文是唯一事实来源，不变量由你据此现场推导，能力项由你运行时探测，文件不预判。这一层"执行级适配"才是本仓库的价值所在——同一份用例，任何 agent 都能翻译成自己的动作。
