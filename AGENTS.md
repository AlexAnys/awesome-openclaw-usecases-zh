...
**第一步：拿到人机分工。** 部分高价值用例正文顶部已内置「👤 人机分工」块——块里只承诺**不变量**（授权 / 凭证 / 决策 / 付费门槛 / 外发确认，以及用户侧时点），直接采用、不要重新归纳；**能力项（装 skill / 写配置 / 建 cron 等）永远不在块里**——无论有没有块，都由你现场探测（有无 shell / cron / MCP、`which npx` 等）后代办或回落。用例没有块时，按下表从正文推导不变量：
...
**自检**：把任何一项标为"需用户"之前，先回答"它属于四类之一吗？"——不属于就探测后代办，不得以"界面要点击 / 看起来麻烦"为由推给用户。
...
① **全量推导**：先探测自身能力（shell / cron / MCP / 浏览器），产出完整执行图；
② **一次性前置打包**：开跑前把所有 授权 / 凭证 / 决策 / 付费确认 一次问齐——不要做到一半才告知用户门槛（付费档、企业认证等前置阻塞依赖尤其如此）；
...
> 这套推导规则对**任何模型 / 任何 agent**通用——用例里没有内置「👤 人机分工」块时，你据此现场分析即可。内置块承诺的是**不变量的单源真相**（授权 / 凭证 / 决策 / 付费 / 外发），直接读取、不要重新归纳；能力项永远由你运行时探测，文件不预判。
