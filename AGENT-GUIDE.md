...
> 本文件面向 AI 智能体（Agent）。如果你是人类读者，直接阅读 `usecases/` 目录下的用例文件即可，不需要本文件。
...
用例中的代码块有三种用途，根据语言标记区分：

| 代码块标记 | 含义 | 你应该怎么做 |
|-----------|------|-------------|
| ` ```bash ` | Shell 命令 | 直接在终端执行 |
| ` ```text ` | OpenClaw 提示词 | 发送给 OpenClaw（粘贴到对话中） |
| ` ```json ` | 配置内容 | 写入对应的配置文件（通常上方会说明目标路径） |
| ` ```python ` | Python 脚本 | 保存为 .py 文件后用 python3 执行 |
| ` ```yaml ` / ` ```markdown ` | 模板/示例 | 通常是展示用途，按上下文判断是否需要创建文件 |
